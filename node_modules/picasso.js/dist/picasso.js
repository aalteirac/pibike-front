...
function keyGen(node, valueFn) {
  return node.ancestors().map(function (a) {
    return valueFn(a.data);
  }).reverse().slice(1) // Delete root node
  .toString();
}

function flattenTree(rootNode, settings) {
  var ticksDepth = _typeof(settings.ticks) === 'object' ? settings.ticks.depth : null;
  var valueFn = typeof settings.value === 'function' ? settings.value : function (d) {
    return d.value;
  };
  ...
  var expando = 0;
  if (!rootNode) {
    return { values: values, labels: labels, items: items, ticks: ticks };
  }

  rootNode.eachAfter(function (node) {
    if (node.depth > 0) {
      var key = keyGen(node, valueFn);
      var leaves = node.leaves() || [node]; // If leaf node returns itself
      var value = valueFn(node.data);
      var label = labelFn(node.data);
...
function scaleHierarchicalBand() {
  ...
  var bandInstance = scaleBand(settings);

  var _flattenTree = flattenTree(data.root, settings),
  ...
  hBand.copy = function () {
    return scaleHierarchicalBand(settings, data);
  };
...
var settingsResolver = function (resources) {
...
var isReservedProperty$1 = function isReservedProperty(prop) {
  return ['on', 'preferredSize', 'created', 'beforeMount', 'mounted', 'resize', 'beforeUpdate', 'updated', 'beforeRender', 'render', 'beforeDestroy', 'destroyed', 'defaultSettings', 'data', 'settings', 'formatter', 'scale', 'chart', 'dockConfig', 'mediator', 'style', 'resolver', 'registries'].some(function (name) {
    return name === prop;
  });
};
...
function getPropsInfo(cfg, dataset) {
  // console.log('222', cfg);
  var _normalizeProperties = normalizeProperties(cfg, dataset, { main: { value: cfg.value, label: cfg.label, reduce: cfg.reduce, filter: cfg.filter } }, {}),
...
function box$1(_ref) {
  ...
  var _resolveDiff = resolveDiff({ start: item.start, end: item.end, minPx: item.box.minHeightPx, maxPx: calcheight }),
...
function buildShapes(_ref4) {
  ...
  var _loop = function _loop(i, len) {
    ...
    if (item.box.show && isNumber(item.start) && isNumber(item.end)) {
      children.push(box$1({ item: item, boxWidth: boxWidth, boxPadding: boxPadding, rendwidth: rendwidth, rendheight: rendheight, flipXY: flipXY }));
    }

    if (item.line.show && isNumber(item.min) && isNumber(item.start)) {
      children.push(verticalLine({ item: item, from: item.min, to: item.start, boxCenter: boxCenter, rendwidth: rendwidth, rendheight: rendheight, flipXY: flipXY }));
    }

    if (item.line.show && isNumber(item.max) && isNumber(item.end)) {
      children.push(verticalLine({ item: item, from: item.max, to: item.end, boxCenter: boxCenter, rendwidth: rendwidth, rendheight: rendheight, flipXY: flipXY }));
    }

    if (item.median.show && isNumber(item.med)) {
      children.push(horizontalLine({ item: item, key: 'median', position: item.med, width: boxWidth, boxCenter: boxCenter, rendwidth: rendwidth, rendheight: rendheight, flipXY: flipXY }));
    }
    ...
    if (item.whisker.show) {
      ...
      if (isNumber(item.min)) {
        children.push(horizontalLine({ item: item, key: 'whisker', position: item.min, width: whiskerWidth, boxCenter: boxCenter, rendwidth: rendwidth, rendheight: rendheight, flipXY: flipXY }));
      }

      if (isNumber(item.max)) {
        children.push(horizontalLine({ item: item, key: 'whisker', position: item.max, width: whiskerWidth, boxCenter: boxCenter, rendwidth: rendwidth, rendheight: rendheight, flipXY: flipXY }));
      }
    }
...
function pointMarker(picasso) {
  picasso.component(type, component$2);
...
function getSize(_ref7) {
  ...
  if (settings.labels.show) {
    ...
    if (isDiscrete && horizontal && settings.labels.mode === 'auto') {
      if (shouldAutoTilt({ majorTicks: majorTicks, measure: measure, rect: rect, state: state, settings: settings })) {
    ...
    if (isDiscrete && state.labels.activeMode !== 'tilted' && isToLarge({ rect: rect, state: state, majorTicks: majorTicks, measure: measure, horizontal: horizontal })) {
  ...
  function build(_ref5) {
    ...
    if (settings.labels.show) {
      ...
      var tickFn = function tickFn(tick) {
        var maxSize = calcMaxTextRectFn({ measureText: measureText, settings: settings, innerRect: innerRect, ticks: ticks, scale: scale, tilted: tilted, layered: layered, tick: tick });
        ...
        buildOpts.stepSize = getStepSizeFn({ innerRect: innerRect, scale: scale, ticks: ticks, settings: settings, tick: tick });
      };
...
function placeTextInRect(rect, text, opts) {
  var label = {
    type: 'text',
    text: text,
    maxWidth: 15,//opts.rotate ? rect.height : rect.width,
    x: 0,
    y: rect.y,
    dx: 0,
    dy: 0,
  ...
  if (opts.rotate) {
    ...
    //label.x = (rect.x + opts.align * wiggleWidth);//-(textMetrics.width);
    label.x = rect.x + (textMetrics.width*2);
    label.y = (rect.y + textMetrics.height / LINE_HEIGHT + opts.justify * wiggleHeight)-15;
  }
...
function placeInBars(_ref3) {
  var chart = _ref3.chart,
      nodes = _ref3.nodes,
      placementSettings = _ref3.placementSettings,
      labelSettings = _ref3.labelSettings,
...
function precalculate(_ref4) {
  ...
  for (var i = 0, len = nodes.length; i < len; i++) {
    ...
    for (var j = 0, lenj = nodes.length; j < lenj; j++) {
      if(i==j)
        continue;
      var lb=nodes[j].localBounds;
      var res=NarrowPhaseCollision.testRectRect(bounds, lb);
      if(res){
        collision=true;
        break;
      }
    ...
    for (var j = 0; j < labelSettings.length; j++) {
      ...
      //measured.height=measured.height-5;measured.width=measured.width-5;
  ...
  render: function render() {
    ...
    (stngs.sources || []).forEach(function (source) {
      if (source.strategy && strategies[source.strategy.type] && source.component) {
        ...
        var stType=strategies[source.strategy.type];
        var stRes=strategy(st, stType)
        labels.push.apply(labels, toConsumableArray(stRes));
      }
    });
...
function placeTextInRect$1(rect, label, opts) {
  var textMetrics = opts.textMetrics;
...
function createRenderItem(_ref) {
  ...
  placeTextInRect$1(labelRect, label, {
...
var itemRendererFactory = function (legend, _ref5) {
...
var navigationRendererFactory = function (legend) {
  var itemized = void 0;
...
var titleRendererFactory = function (legend) {
  var itemized = void 0;
...
function update(comp) {
  comp.state.resolved = resolveSettings$1(comp);
...
function categoricalLegend(picasso) {
  picasso.component(type$2, component$3);
}
...
var components = [box, pointMarker, pie$1, gridLine, refLine, axis, text, scrollbar, rangeBrush, rangeBrush$1, lassoBrush, labels, categoricalLegend, sequentialLegend, pointMarker$1];
  ...
  createClass(GeometryCollection, [{
    ...
    value: function set$$1() {
      ...
      collection.forEach(function (geo) {
        var geoInstance = create$7(geo.type, geo);
        if (geoInstance) {
  ...
  createClass(Container, [{
    ...
    value: function addChild(c) {
      ...
      if (this._collider && this._collider.type === 'bounds') {
        ...
        var opts = extend({ type: 'bounds', x: 0, y: 0, width: 0, height: 0 }, this._boundingRect);
        get(Container.prototype.__proto__ || Object.getPrototypeOf(Container.prototype), 'collider', this).call(this, opts);
      }
...
var Rect = function (_DisplayObject) {
  ...
  createClass(Rect, [{
    ...
    value: function set$$1() {
      ...
      var opts = extend({ type: 'rect', x: x, y: y, width: width, height: height }, collider);
...
var Circle = function (_DisplayObject) {
  ...
  createClass(Circle, [{
    ...
    value: function set$$1() {
      ...
      var opts = extend({ type: 'circle', cx: cx, cy: cy, r: r }, collider);
...
reg$4.add('rect', create$17);
reg$4.add('circle', create$18);
reg$4.add('text', create$21);
reg$4.add('line', create$19);
reg$4.add('path', create$20);
reg$4.add('stage', create$5);
reg$4.add('container', create$6);
reg$4.add('defs', create$16);
reg$4.add('linearGradient', create$16);
reg$4.add('radialGradient', create$16);
reg$4.add('stop', create$16);
